...
function crearTabla(datos){
		...
		if(totalReclamos > 0){
			let texto = document.createTextNode("El promedio de las cantidades considerando todos los reclamos de todas las empresas es: " + Math.trunc((sistema.cantidadTotal()/totalReclamos)));
			promedioEmpresas.appendChild(texto);
		}else{
			let texto = document.createTextNode("El promedio de las cantidades considerando todos los reclamos de todas las empresas es: " + 0);
			promedioEmpresas.appendChild(texto);
		}
...
function crearListaEmpresaSinReclamo(){
	...
	lista.innerHTML = "";
	let info = sistema.darTodasE();
	for(let i = 0; i < info.length ; i++){
		if(info[i].contadorDeReclamo == 0){
			let li = document.createElement("li");
			let nodoT = document.createTextNode(info[i].nombre + " (" 
			+ info[i].direccion + ") Rubro: " + info[i].rubro);
